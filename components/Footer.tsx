...
export default function Footer({ locale = "pt" }: { locale?: "pt" | "en" | "es" }) {
  ...
  return (
    <footer className="bg-[#0a1a2f] text-white/90 border-t border-white/10 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-3 gap-10">
        ...
        <div className="space-y-5">
          ...
          <button
            onClick={() =>
              isMobile()
                ? (window.location.href = "tel:3133718500")
                : handleCopy("(31) 3371-8500")
            }
        ...
        <div className="space-y-4">
          ...
          <ul className="text-sm space-y-3">
            ...
            <li className="flex justify-between">
              <span>{t.sunday}:</span>
              <span className="text-[#ff4d4d] font-medium">{t.closed}</span>
            </li>
          </ul>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <a
              href="mailto:loja@grupoprotect.com.br"
              className="flex items-center gap-3 text-sm hover:text-[#ffc300ff] transition-colors"
            >
              <FaEnvelope className="text-white/40" size={14} />
              loja@grupoprotect.com.br
            </a>
            <a
              href="mailto:vendas@grupoprotect.com.br"
              className="flex items-center gap-3 text-sm hover:text-[#ffc300ff] transition-colors"
            >
              <FaEnvelope className="text-white/40" size={14} />
      ...
      <div className="bg-[#020b18] py-6 px-6 text-center text-[11px] text-white/40 tracking-widest">
        © {new Date().getFullYear()} GRUPO PROTECT — {t.rights}
      </div>

      {/* TOAST */}
      <div className="fixed top-8 right-1/2 translate-[50%] z-40 pointer-events-none">
        <div
