...
import { dictionaries } from "../i18n/indes";

const SUPPORTED_LANGS = ["pt", "en", "es"] as const;
type Lang = (typeof SUPPORTED_LANGS)[number];

type MenuItem = {
  key: keyof typeof dictionaries.pt.navbar;
  href: string;
  submenu?: string[];
};
...
export default function NavBar() {
  ...
  const t = dictionaries[currentLang].navbar;

  // 🔄 Troca idioma mantendo rota
  const changeLanguage = (lang: Lang) => {
    const segments = pathname.split("/").filter(Boolean);
    if (SUPPORTED_LANGS.includes(segments[0] as Lang)) segments[0] = lang;
  ...
  const menuItems: MenuItem[] = [
    ...
    { key: "courses", href: "/cursos", submenu: t.coursesMenu },
    { key: "products", href: "/produtos", submenu: t.productsMenu },
    { key: "dispatcher", href: "/despachante", submenu: t.dispatcherMenu },
    { key: "club", href: "/clube", submenu: t.clubMenu },
    { key: "blog", href: "/blog" },
    { key: "training", href: "/treinamento", submenu: t.trainingMenu },
    { key: "join", href: "/associe-se" },
  ...
  const searchLibrary = useMemo(() => {
    ...
    menuItems.forEach((item) => {
      list.push({ name: t[item.key] as string, href: item.href, category: "Menu" });
      if (item.submenu) {
        item.submenu.forEach((sub) => {
          list.push({ name: sub, href: item.href, category: t[item.key] as string });
        });
      }
    });
    return list;
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (query.trim()) {
      router.push(`/${currentLang}/search?q=${encodeURIComponent(query)}`);
      setQuery("");
      setSearchOpen(false);
      setIsFocused(false);
    }
    ...
    <nav className="bg-[#003566] p-4 relative z-50 w-full border-b border-white/10 shadow-md">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="text-white flex items-center gap-6">
          <Image
            src="/logo-horizontal-branco2-removebg-preview.png"
            alt="Logo"
            width={110}
            height={45}
          ...
          <ul className="hidden lg:flex space-x-5 text-[13px] font-medium">
            {menuItems.map((item) => (
              <li key={item.key as string} className="relative group py-2">
                <a
                  href={`/${currentLang}${item.href}`}
                  className="text-[#f2f2f2]/80 hover:text-white transition-all flex items-center gap-1"
                >
                  {t[item.key]}
                  {item.submenu && (
                    <ChevronDown
                  ...
                  <ul className="absolute left-0 top-full hidden group-hover:block bg-white min-w-55 shadow-2xl border-t-[3px] border-[#ffb703] py-2 animate-in fade-in slide-in-from-top-1">
                    {item.submenu.map((sub) => (
                      <li key={sub}>
                        <a
                          href={`/${currentLang}${item.href}`}
                          className="block px-4 py-2 text-[#003566] hover:bg-[#003566] hover:text-white text-[13px] transition-colors"
                        >
                          {sub}
                        </a>
                      </li>
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="relative hidden xl:block" ref={searchRef}>
            <form
              ...
              className={`flex items-center bg-black/20 rounded-full px-3 py-1.5 border transition-all ${
                isFocused ? "border-[#ffb703] bg-black/30 w-64" : "border-white/10 w-48"
              }`}
            >
              <input
                type="text"
                placeholder={currentLang === "pt" ? "Buscar no site..." : currentLang === "en" ? "Search site..." : "Buscar en el sitio..."}
            ...
            {isFocused && filteredResults.length > 0 && (
              <div className="absolute top-full right-0 mt-2 w-full bg-white rounded-lg shadow-2xl border border-gray-200 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                {filteredResults.map((res, i) => (
                  <a
                    key={i}
                    href={`/${currentLang}${res.href}`}
                    onClick={() => setQuery("")}
                    className="block px-4 py-2.5 hover:bg-gray-50 border-b last:border-0 border-gray-100"
                  >
                    <p className="text-[10px] text-[#ffb703] font-bold uppercase">
                      {res.category}
                    </p>
                ...
                return (
                  <button
                    ...
                    className={`relative hover:scale-110 transition-transform cursor-pointer ${
                      isActive ? "ring-2 ring-[#ffb703] rounded-sm" : ""
                    }`}
                  >
                    <img
                      src={lang.src}
                    ...
                    />
                    {isActive && (
                      <span className="absolute -bottom-1 -right-1 bg-[#ffb703] rounded-full p-0.5">
                        <Check size={10} className="text-[#003566]" />
                      </span>
                    )}
                  </button>
                );
        ...
        <div className="absolute top-full left-0 w-full bg-[#002855] border-b border-[#ffb703] p-4 lg:hidden animate-in slide-in-from-top duration-300">
          <form onSubmit={handleSearchSubmit} className="flex gap-2">
            <input
              autoFocus
              type="text"
              placeholder={currentLang === "pt" ? "O que você busca?" : currentLang === "en" ? "What are you looking for?" : "¿Qué estás buscando?"}
            ...
            <div className="mt-4 space-y-2">
              {filteredResults.map((res, i) => (
                <a
                  key={i}
                  href={`/${currentLang}${res.href}`}
                  className="block p-3 bg-white/5 rounded-md text-white border-l-2 border-[#ffb703]"
                >
      ...
      <div
        className={`lg:hidden overflow-hidden transition-all duration-500 ease-in-out ${
          open ? "max-h-500 opacity-100 mt-4" : "max-h-0 opacity-0 pointer-events-none"
        }`}
      >
        <div className="bg-[#002855] rounded-xl border border-white/10 p-4 space-y-2">
          {menuItems.map((item) => (
            <div key={item.key as string}>
              <a
                href={`/${currentLang}${item.href}`}
                className="block py-2 text-white font-medium border-b border-white/5"
              >
                {t[item.key]}
              </a>
              {item.submenu && (
                <div className="pl-4 py-1 space-y-1 mt-1">
                  {item.submenu.map((sub) => (
                    <a
                      key={sub}
                      href={`/${currentLang}${item.href}`}
                      className="block py-1 text-white/60 text-sm hover:text-[#ffb703]"
                    >
                      {sub}
                    </a>
                  ))}
                ...
                >
                  <img
                    ...
                    className={`w-8 rounded-sm shadow-md ${
                      isActive ? "ring-2 ring-[#ffb703]" : ""
                    }`}
                  />
                  <span className="text-[10px] text-white/50">{lang.code}</span>
                </button>
              );
            })}
          </div>
...
function IconFacebook() {
  return (
    <a href="#" className="text-white hover:text-[#ffb703] transition-colors">
