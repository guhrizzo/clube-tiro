...
export default function NavBar() {
  ...
  useEffect(() => {
    ...
    const normalized = browserLang.startsWith("pt") ? "pt-BR" :
  ...
  useEffect(() => {
    ...
    if (SUPPORTED_LANGS.includes(segments[0])) {
      segments[0] = activeLang;
    } else {
      segments.unshift(activeLang);
    }
  ...
  const languages = [
    { code: "pt-BR", label: "Português", src: "https://purecatamphetamine.github.io/country-flag-icons/3x2/BR.svg" },
              ...
              {languages.map((lang) => {
                ...
                return (
                  <button
                    ...
                    className={`relative hover:scale-110 transition-transform cursor-pointer ${isActive ? "ring-2 ring-[#ffb703] rounded-sm" : ""}`}
                  >
                    <img src={lang.src} alt={lang.label} className={`w-5 h-auto rounded-sm ${isActive ? "opacity-100" : "opacity-70 hover:opacity-100"}`} />
                    {isActive && (
            ...
            {languages.map((lang) => {
              ...
              return (
                <button
                  key={lang.code}
                  onClick={() => changeLanguage(lang.code)}
                  className={`flex flex-col items-center gap-1 ${isActive ? "scale-110" : "opacity-70"}`}
                >
                  <img src={lang.src} alt={lang.label} className={`w-8 rounded-sm shadow-md ${isActive ? "ring-2 ring-[#ffb703]" : ""}`} />
...
function IconTwitter() {
  return <a href="#" className="text-white hover:text-[#ffb703] transition-colors"><svg width="18" height="18" fill="currentColor" viewBox="0 0 16 16"><path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865z" /></svg></a>;
}
