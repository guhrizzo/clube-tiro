    ...
    <div className="space-y-1.5" ref={ref}>
      ...
      <div className="relative">
        ...
        >
          ...
          <span className="text-xs text-slate-400 hidden sm:block truncate ml-1">{selected.address}</span>
          <ChevronDown
            size={15}
              ...
              {EMAILS.map((dept) => (
                <li key={dept.address}>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors hover:bg-slate-50 cursor-pointer ${selected.address === dept.address ? "bg-slate-50" : ""}`}
                  >
                    <span
                      className="shrink-0 w-2.5 h-2.5 rounded-full"
                      style={{ backgroundColor: dept.color, boxShadow: `0 0 8px ${dept.color}80` }}
                    />
                    <span className="font-bold text-slate-800">{dept.dept}</span>
                    <span className="text-xs text-slate-400 truncate">{dept.address}</span>
                    {selected.address === dept.address && <Check size={14} className="ml-auto shrink-0 text-emerald-500" />}
                  </button>
                </li>
              ))}
            </motion.ul>
...
export default function ContactPremium() {
  ...
  const [formData, setFormData] = useState({ nome: "", email: "", telefone: "", plano: t.planIndividual, mensagem: "" });
          ...
          <div className="relative z-10 grid lg:grid-cols-2 gap-0">
            ...
            <div className="p-8 md:p-12 lg:p-14 space-y-8 border-b lg:border-b-0 lg:border-r border-slate-100">
              <div>
                ...
                <h2 className="text-3xl md:text-4xl font-black text-slate-900 leading-tight mb-2">{t.title}</h2>
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-0.5">{t.phone}</p>
                  <a href="tel:+553133718500" className="text-slate-900 font-bold text-sm hover:text-[#ffb703] transition-colors">
                    +55 (31) 3371-8500
                  </a>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-600 ml-1 uppercase tracking-wider">{t.labelName}</label>
                    <input required type="text" value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })} placeholder={t.placeholderName} className={inputClass} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-600 ml-1 uppercase tracking-wider">{t.labelEmail}</label>
                    <input required type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder={t.placeholderEmail} className={inputClass} />
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-600 ml-1 uppercase tracking-wider">{t.labelPhone}</label>
                  <input required type="tel" value={formData.telefone} onChange={(e) => setFormData({ ...formData, telefone: e.target.value })} placeholder={t.placeholderPhone} className={inputClass} />
                </div>

                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-600 ml-1 uppercase tracking-wider">{t.labelPlan}</label>
                  <div className="flex flex-col sm:flex-row gap-3">
                    {[t.planIndividual, t.planBusiness].map((opt) => (
                      <label
                        key={opt}
                        className={`flex flex-1 items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer text-xs font-bold ${formData.plano === opt ? "border-[#ffb703] bg-[#ffb703]/5 text-slate-900" : "border-slate-200 bg-white text-slate-500 hover:border-slate-300"}`}
                      >
                        <input type="radio" name="plano" checked={formData.plano === opt} onChange={() => setFormData({ ...formData, plano: opt })} className="accent-[#ffb703] w-4 h-4" />
                        {opt}
                      </label>
                    ))}
                  </div>
                ...
                <DeptSelector selected={selectedDept} onSelect={setSelectedDept} label={t.labelDept} />
                ...
                <motion.div
                  ...
                  style={{ borderColor: `${selectedDept.color}40`, backgroundColor: `${selectedDept.color}08`, color: selectedDept.color }}
                >
                  <Mail size={13} />
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-600 ml-1 uppercase tracking-wider">{t.labelMessage}</label>
                  <textarea required rows={4} value={formData.mensagem} onChange={(e) => setFormData({ ...formData, mensagem: e.target.value })} placeholder={t.placeholderMessage} className={`${inputClass} resize-none`} />
                </div>
