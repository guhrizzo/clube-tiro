...
import { X, CheckCircle, FileText, Lock, Printer, Download, Mail, Building2, PenLine, AlignLeft } from "lucide-react";
...
// ── Signature Block ───────────────────────────────────────────────────────────
function SigBlock({
  imgSrc, imgAlt, label, lines, borderColor = "border-gray-400",
}: {
  imgSrc?: string; imgAlt?: string; label?: string; lines: string[]; borderColor?: string;
}) {
  return (
    <div>
      <div className="h-16 flex items-end pb-1">
        {imgSrc && (
          <img src={imgSrc} alt={imgAlt} crossOrigin="anonymous"
      ...
      <div className={`border-t ${borderColor} pt-2`}>
        ...
        {lines.map((line, i) => (
          <p key={i} className={i === 0 && !label ? "font-bold" : i > 0 ? "text-gray-500" : ""}>{line}</p>
        ))}
      </div>
    </div>
  );
...
// ── Success Modal ─────────────────────────────────────────────────────────────
function SuccessModal({ email, onClose, onPrint, onSave }: {
    ...
    <div className="fixed inset-0 z-10000 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-white rounded-sm shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="bg-gray-900 px-6 pt-8 pb-6 text-center">
          ...
          <h2 className="text-white text-base font-semibold tracking-tight">Contrato assinado com sucesso!</h2>
          <p className="text-gray-400 text-xs mt-1">Seu cadastro foi registrado na PROTECT.</p>
        </div>
        <div className="px-6 py-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            <button onClick={onPrint} className="flex items-center justify-center gap-1.5 py-2.5 border border-gray-200 text-gray-700 text-xs font-medium hover:bg-gray-50 transition-colors rounded-sm cursor-pointer">
              <Printer size={13} /> Imprimir
            </button>
            <button onClick={onSave} className="flex items-center justify-center gap-1.5 py-2.5 border border-gray-200 text-gray-700 text-xs font-medium hover:bg-gray-50 transition-colors rounded-sm cursor-pointer">
              <Download size={13} /> Salvar PDF
            </button>
          </div>
          <button onClick={onClose} className="w-full py-3 bg-gray-900 text-white text-xs font-medium tracking-widest uppercase hover:bg-gray-700 transition-all rounded-sm cursor-pointer mt-1">
            Fechar
          </button>
        </div>
      </div>
...
export default function ContractModal({ isOpen, onClose }: ContractModalProps) {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (saved) {
          ...
          // Migra yyyy-mm-dd → dd/mm/aaaa se vier do formato antigo
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(formData)); } catch { /* ignore */ }
    }
  }, [formData]);
  ...
  const cloneWithComputedStyles = (): HTMLElement => {
    ...
    const PROPS = [
      ...
      "border-color","border-style","border-width","padding-top","padding-right",
      "padding-bottom","padding-left","margin-top","margin-right","margin-bottom",
      "margin-left","text-align","text-transform","text-decoration","letter-spacing",
    ...
    const applyStyles = (orig: Element, clone: Element) => {
      if (orig instanceof HTMLElement && clone instanceof HTMLElement) {
        ...
        PROPS.forEach((prop) => {
          ...
          if (val) (clone.style as unknown as Record<string, string>)[toCamel(prop)] = val;
        });
      }
      Array.from(orig.children).forEach((child, i) => applyStyles(child, clone.children[i]));
    };
...
<style>*{box-sizing:border-box;}body{margin:0;padding:32px;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;}img{max-height:56px!important;max-width:180px!important;width:auto!important;height:auto!important;display:block;}@media print{body{padding:0;}@page{margin:16mm;size:A4;}}</style>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const opt = {
        margin: [12, 14, 12, 14],
        filename: `Contrato_PROTECT_${(formData.nome || "socio").replace(/\s+/g, "_").normalize("NFD").replace(/[\u0300-\u036f]/g, "")}.pdf`,
  ...
  const handleDateChange = (raw: string) => {
    ...
    setFormData({ ...formData, nascimento: masked });
    ...
    <div className="flex-[1.6] flex flex-col overflow-hidden border-r border-gray-200">
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button type="button" onClick={handlePrint} title="Imprimir contrato"
          ...
          <button type="button" onClick={handleSave} disabled={savingPdf} title="Salvar como PDF"
            ...
            {savingPdf ? (
              <><div className="w-3 h-3 border border-gray-500 border-t-transparent rounded-full animate-spin" /><span className="hidden sm:inline">Gerando...</span></>
            ) : (
              <><Download size={13} /><span className="hidden sm:inline">Salvar PDF</span></>
            )}
          </button>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 px-4 md:px-6 py-8" ref={scrollRef}>
        <div ref={contractRef}
          ...
          <p className="mb-4 text-justify">
            Pelo presente instrumento particular de <strong>CONTRATO DE ADESÃO DE SÓCIO USUÁRIO (COLABORADOR)</strong>, de um lado,{" "}
            ...
            e posteriormente na Rua dos Radialistas, 38, Bairro Balneário Água Limpa, Nova Lima/MG, neste ato representada por quem
            de direito, doravante simplesmente denominada <strong>PROTECT</strong>; e, de outro lado, o(a) Sr.(a){" "}
            ...
            {blank("dd/mm/aaaa", nascimentoDisplay, "min-w-[90px]")},
            doravante simplesmente denominado(a) <strong>SÓCIO USUÁRIO (COLABORADOR)</strong>, têm entre si justo e contratado o
            direito de sócio usuário (colaborador) da PROTECT, tudo de acordo com as condições especificadas nesta
          ...
          <div className="border-t border-gray-200 pt-8 mt-8 space-y-8 text-xs">
            ...
            <div className="grid grid-cols-2 gap-8">
              <SigBlock imgSrc="/assinatura2.png" imgAlt="Assinatura Protect" borderColor="border-gray-500"
                lines={["PROTECT CLUBE MINEIRO DE TIRO", "CNPJ: 01.244.200/0001-52", "ANTONIO C. COSTA JUNIOR"]} />
              <SigBlock borderColor="border-gray-500"
                lines={[formData.nome || "SÓCIO USUÁRIO (COLABORADOR)", `CPF: ${formData.cpf || "___________________________"}`]} />
            </div>
            <div className="grid grid-cols-2 gap-8">
              <SigBlock imgSrc="/assinatura1.png" imgAlt="Assinatura Testemunha 1" label="TESTEMUNHA 1"
                lines={["EMMERSON N. DO CARMO", "CPF: 001.583.866-80"]} />
              <SigBlock imgSrc="/assinatura3.png" imgAlt="Assinatura Testemunha 2" label="TESTEMUNHA 2"
            ...
            <p className="text-center text-gray-400">
              Rua General Andrade Neves, 622, Grajaú, CEP 30431-128 — Belo Horizonte/MG<br />
      ...
      <form onSubmit={handleConfirm} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              return (
                <button key={key} type="button" onClick={() => { setPlano(key); setAccepted(false); }}
                  className={`flex flex-col items-start px-3 py-2.5 border text-left transition-all cursor-pointer rounded-sm ${
                    active ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 hover:border-gray-400 text-gray-700"
                  }`}>
        ...
        <div className="pt-2 space-y-4">
          <label className="flex items-start gap-2.5 cursor-pointer select-none p-3 border border-gray-100 bg-gray-50 rounded-sm hover:bg-gray-100 transition-colors">
            ...
            <span className="text-xs text-gray-600 leading-relaxed">
              Li e concordo integralmente com todos os termos e cláusulas do contrato, incluindo as condições de pagamento, rescisão e responsabilidades.
            </span>
          </label>
        ...
        <div className="border-t border-gray-100 pt-4 mt-2">
          ...
          <div className="bg-gray-50 border border-gray-100 rounded-sm px-3 py-2.5 space-y-1.5 text-xs text-gray-600">
            {[["Plano", `${plano} anos`], ["Valor total", p.total], ["1ª parcela", p.parcela], ["Vigência", p.vigencia]].map(([k, v]) => (
          ...
          <div className="border-t border-gray-100 pt-4 mt-4">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-full border border-gray-100 bg-gray-50 rounded-sm px-3 py-2.5 space-y-1.5 text-[11px] text-gray-600">
                ...
                {[["Banco", "Bradesco"], ["Agência", "2522-4 (Nova Suíça)"], ["Conta", "118453-9"]].map(([k, v]) => (
                  <div key={k} className="flex justify-between">
        ...
        {/* Padding extra no final para não cortar conteúdo no mobile */}
        <div className="h-8" />
      </form>
    </div>
  );
      ...
      <div className="fixed inset-0 z-9999 flex items-center justify-center bg-black/60 p-0 md:p-6"
        ...
        <div ref={modalRef} tabIndex={-1}
          ...
          <div className="md:hidden flex shrink-0 border-b border-gray-200 bg-white">
            <button type="button" onClick={() => setMobileTab("contrato")}
              className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-xs font-semibold uppercase tracking-widest transition-colors cursor-pointer ${
                mobileTab === "contrato" ? "text-gray-900 border-b-2 border-gray-900" : "text-gray-400 hover:text-gray-600"
              }`}>
              <AlignLeft size={14} /> Contrato
            </button>
            <button type="button" onClick={() => setMobileTab("dados")}
              className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-xs font-semibold uppercase tracking-widest transition-colors cursor-pointer ${
                mobileTab === "dados" ? "text-gray-900 border-b-2 border-gray-900" : "text-gray-400 hover:text-gray-600"
              }`}>
          ...
          <div className="md:hidden flex-1 overflow-y-auto flex flex-col">
            {mobileTab === "contrato" ? <DocumentPanel /> : <FormPanel />}
          </div>
