...
function SuccessModal({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-sm shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 pointer-events-auto">
        ...
        <div className="px-6 py-5 space-y-3">
          <div className="flex items-start gap-3 p-3 border border-gray-100 rounded-sm bg-gray-50">
            ...
            <div>
              <p className="text-xs font-semibold text-gray-800">Cópia enviada para você</p>
              <p className="text-[11px] text-gray-500 mt-0.5 font-mono break-all">{email}</p>
            </div>
          </div>
          <div className="flex items-start gap-3 p-3 border border-gray-100 rounded-sm bg-gray-50">
            ...
            <div>
              <p className="text-xs font-semibold text-gray-800">Cópia enviada ao Clube PROTECT</p>
              <p className="text-[11px] text-gray-500 mt-0.5">clube@grupoprotect.com.br</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 pt-1">
            ...
            <button type="button" onClick={onSave} disabled={saving}
              className={`flex items-center justify-center gap-1.5 py-2.5 border text-xs font-medium transition-all rounded-sm ${saving ? "opacity-50 cursor-not-allowed border-gray-100 text-gray-300 bg-gray-50" : "border-gray-300 text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-400 cursor-pointer"}`}>
              <svg className="w-[13px] h-[13px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function ContractModal({ isOpen, onClose }: ContractModalProps) {
  ...
  // Controla se o portal pode ser renderizado (SSR safe)
  const [isMounted, setIsMounted] = useState(false);

  const scrollRef = useRef<HTMLDivElement>(null);
  const modalRef = useRef<HTMLDivElement>(null);

  // Ref do contrato visível na UI (mobile tab ou desktop side-by-side)
  const contractRefVisible = useRef<HTMLDivElement>(null);

  // Ref do elemento fantasma no portal — SEMPRE tem width:672px real.
  // Todo PDF/impressão usa este ref, garantindo 4 páginas A4 no mobile e desktop.
  const contractRefPDF = useRef<HTMLDivElement>(null);
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (typeof window !== "undefined") {
      try {
        ...
        if (saved) {
          ...
          if (parsed.nascimento && /^\d{4}-\d{2}-\d{2}$/.test(parsed.nascimento)) {
    ...
    return { nome: "", email: "", cpf: "", rg: "", profissao: "", naturalidade: "", nascimento: "" };
  });
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const contractElement = contractRefPDF.current;
        if (!contractElement) throw new Error("Elemento PDF não encontrado");
  ...
  const nascimentoDisplay = formData.nascimento.length === 10 ? formData.nascimento : "";

  const field = "w-full px-2 py-1.5 border-b border-gray-300 bg-transparent text-sm outline-none focus:border-gray-800 transition-colors placeholder:text-gray-300";
  const labelCls = "block text-[10px] uppercase tracking-widest text-gray-400 mb-1 font-medium";

  const blank = (label: string, value: string, width = "min-w-[180px]") =>
    value ? (
      <span className={`inline-block border-b border-gray-300 px-1 font-semibold ${width}`}>{value}</span>
    ) : (
      <span className={`inline-block border-b border-gray-200 px-1 text-gray-300 ${width}`}>{label}</span>
    );

  // Conteúdo do contrato — reutilizado tanto na UI visível quanto no elemento fantasma do portal
  const contractContent = (
    <div className="max-w-2xl mx-auto bg-white border border-gray-200 px-6 md:px-10 py-12 text-[13px] text-gray-800 leading-relaxed font-serif shadow-sm">
      ...
      <p className="mb-4 text-justify">
        ...
        <strong>Protect Clube Mineiro de Tiro</strong>, pessoa jurídica de direito privado, inscrita no CNPJ sob o nº{" "}
        <strong>01.244.200/0001-52</strong>, com sede na Rua General Andrade Neves, 622, Bairro Gutierrez, Belo Horizonte/MG,
        e posteriormente na Rua dos Radialistas, 38, Bairro Balneário Água Limpa, Nova Lima/MG, neste ato representada por
        quem de direito, doravante simplesmente denominada <strong>PROTECT</strong>; e, de outro lado, o(a) Sr.(a){" "}
        ...
        {blank("dd/mm/aaaa", nascimentoDisplay, "min-w-[90px]")}, doravante simplesmente denominado(a){" "}
        <strong>SÓCIO USUÁRIO (COLABORADOR)</strong>, têm entre si justo e contratado o direito de sócio usuário (colaborador)
        da PROTECT, tudo de acordo com as condições especificadas nesta contratação/adesão e na legislação vigente.
      </p>
      <p className="mb-6 text-justify">
        O proponente declara aceitar as cláusulas deste contrato sem restrições, bem como a eleição do foro da Comarca de
        Belo Horizonte/MG para dirimir quaisquer pendências relativas ao presente instrumento.
      </p>

      <h2 className="font-bold text-sm uppercase mb-3 mt-6">1. Do Preço e da Forma de Pagamento</h2>
      <p className="mb-3"><strong>CLÁUSULA PRIMEIRA:</strong> O valor total do contrato, correspondente à cota {p.nomeContrato} (vigente por {p.nomePrazo} anos), é de <strong>{p.total}</strong>, ficando isento o pagamento de taxa de contribuição social mensal.</p>
      ...
      <p className="mb-3"><strong>CLÁUSULA ONZE:</strong> Pagar pontualmente as parcelas para quitação do título {p.nomeContrato} de sócio usuário (colaborador), quando parcelado.</p>
      ...
      <p className="mb-3"><strong>CLÁUSULA DEZOITO:</strong> O sócio declara ter ciência de que CACs <strong>não possuem autorização para PORTE de arma</strong>, e sim, alguns deles, para POSSE e TRANSPORTE da arma, munições e acessórios de casa até os locais de competição, clubes de tiro, treinamentos e outros, autorizados pelas GUIAS DE TRÁFEGO.</p>
      ...
      <p className="mb-3"><strong>PARÁGRAFO SEGUNDO:</strong> Em caso de rescisão ou cancelamento deste contrato, a parte que der ensejo à rescisão deverá pagar à outra parte <strong>30% (trinta por cento)</strong> do valor da cota {p.nomeContrato} de sócio usuário (colaborador), independentemente da entrada e/ou das parcelas pagas, quantia que será liquidada no ato da rescisão ou do cancelamento.</p>
      ...
      <div className="border-t border-gray-200 pt-8 mt-8 space-y-8 text-xs">
        ...
        <div className="grid grid-cols-2 gap-8">
          <SigBlock imgSrc="/assinatura2.png" imgAlt="Assinatura Protect" lines={["PROTECT CLUBE MINEIRO DE TIRO", "CNPJ: 01.244.200/0001-52", "ANTONIO C. COSTA JUNIOR"]} />
          <SigBlock lines={[formData.nome || "SÓCIO USUÁRIO (COLABORADOR)", `CPF: ${formData.cpf || "___________________________"}`]} />
        </div>
        <div className="grid grid-cols-2 gap-8">
          <SigBlock imgSrc="/assinatura1.png" imgAlt="Assinatura Testemunha 1" label="TESTEMUNHA 1" lines={["EMMERSON N. DO CARMO", "CPF: 001.583.866-80"]} />
          <SigBlock imgSrc="/assinatura3.png" imgAlt="Assinatura Testemunha 2" label="TESTEMUNHA 2" lines={["NEWTON C. BAPTISTON", "CPF: 584.978.896-49"]} />
        </div>
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button type="button" onClick={handleSavePDF} disabled={saving}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] transition rounded-sm ${saving ? "text-gray-300 bg-gray-100 opacity-50 cursor-not-allowed" : "text-gray-900 bg-gray-100 hover:bg-gray-200 cursor-pointer"}`}>
            <svg className="w-[13px] h-[13px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
            </svg>
            <span className="hidden sm:inline">{saving ? "..." : "Salvar PDF"}</span>
          </button>
          <div className="w-px h-4 bg-gray-200 mx-1 hidden md:block" />
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 transition p-1 cursor-pointer hidden md:block" aria-label="Fechar">
            <X size={18} />
          </button>
        </div>
      </div>
  ...
  const renderFormPanel = () => (
    <div className="w-full md:w-85 shrink-0 flex flex-col bg-white border-t md:border-t-0 border-gray-200">
      <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 shrink-0">
        <h3 className="text-sm font-semibold text-gray-800 tracking-tight">Dados do Assinante</h3>
        <p className="text-xs text-gray-400 mt-0.5">Preencha para inserção no contrato.</p>
      </div>
      ...
      <form onSubmit={handleConfirm} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        <div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-1">
            {(Object.keys(PLANOS) as PlanoKey[]).map((key) => {
              ...
              return (
                <button key={key} type="button" onClick={() => { setPlano(key); setAccepted(false); }}
                  className={`flex flex-col items-start px-3 py-2.5 border text-left transition-all cursor-pointer rounded-sm ${active ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 hover:border-gray-400 text-gray-700"}`}>
                  <span className="text-xs font-bold uppercase tracking-wider">{op.label}</span>
                  <span className={`text-[11px] mt-0.5 ${active ? "text-gray-300" : "text-gray-400"}`}>{op.parcela}/ano</span>
                  <span className={`text-[10px] mt-1 font-semibold ${active ? "text-gray-200" : "text-gray-500"}`}>Total: {op.total}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex items-center gap-2 pt-1">
          <span className="text-[10px] uppercase tracking-widest text-gray-300 whitespace-nowrap">Identificação</span>
          <div className="flex-1 h-px bg-gray-100" />
        </div>

        <div>
          <label className={labelCls}>Nome completo</label>
          <input required type="text" placeholder="Como consta no RG" className={field} value={formData.nome}
            onChange={(e) => setFormData((prev) => ({ ...prev, nome: e.target.value.toUpperCase() }))} />
        </div>
        <div>
          <label className={labelCls}>Profissão</label>
          <input required type="text" placeholder="Sua profissão" className={field} value={formData.profissao}
            onChange={(e) => setFormData((prev) => ({ ...prev, profissao: e.target.value }))} />
        </div>
        <div className="grid grid-cols-1 gap-4 min-w-0">
          <div className="min-w-0">
            <label className={labelCls}>RG</label>
            <input required type="text" placeholder="MG-00.000.000" className={field} value={formData.rg}
              onChange={(e) => setFormData((prev) => ({ ...prev, rg: e.target.value }))} />
          </div>
          <div className="min-w-0">
            <label className={labelCls}>CPF</label>
            <input required type="text" placeholder="000.000.000-00" className={field} maxLength={14} value={formData.cpf}
        ...
        <div className="grid grid-cols-1 gap-4 min-w-0">
          <div className="min-w-0">
            ...
            <input required type="text" placeholder="Cidade/UF" className={field} value={formData.naturalidade}
              onChange={(e) => setFormData((prev) => ({ ...prev, naturalidade: e.target.value }))} />
          </div>
          <div className="min-w-0">
            <label className={labelCls}>Nascimento — dd/mm/aaaa</label>
            <input required type="text" inputMode="numeric" placeholder="dd/mm/aaaa" maxLength={10}
              className={`${field} ${dateError ? "border-red-400 focus:border-red-500" : ""}`}
              value={formData.nascimento} onChange={(e) => handleDateChange(e.target.value)}
              onBlur={() => { if (formData.nascimento.length > 0 && formData.nascimento.length < 10) setDateError("Data incompleta"); }} />
            {dateError && <p className="text-[10px] text-red-500 mt-1">{dateError}</p>}
          </div>
        </div>

        <div className="flex items-center gap-2 pt-1">
          <span className="text-[10px] uppercase tracking-widest text-gray-300 whitespace-nowrap">Contato</span>
          <div className="flex-1 h-px bg-gray-100" />
        </div>

        <div>
          <label className={labelCls}>E-mail</label>
          <input required type="email" placeholder="seu@email.com" className={field} value={formData.email}
        ...
        <div className="pt-2 space-y-4">
          <label className="flex items-start gap-2.5 cursor-pointer select-none p-3 border border-gray-100 bg-gray-50 rounded-sm hover:bg-gray-100 transition-colors">
            ...
            <span className="text-xs text-gray-600 leading-relaxed">
              Li e concordo integralmente com todos os termos e cláusulas do contrato, incluindo as condições de pagamento,
              rescisão e responsabilidades. Ao assinar, seus dados serão registrados com segurança e protegidos de acordo
              com a LGPD (Lei nº 13.709/2018).
            </span>
          </label>
          <button type="submit" disabled={loading || !accepted || !!dateError}
        ...
        <div className="border-t border-gray-100 pt-4 mt-2">
          ...
          <div className="bg-gray-50 border border-gray-100 rounded-sm px-3 py-2.5 space-y-1.5 text-xs text-gray-600">
            {([["Plano", `${plano} anos`], ["Valor total", p.total], ["1ª parcela", p.parcela], ["Vigência", p.vigencia]] as const).map(([k, v]) => (
          ...
          <div className="border-t border-gray-100 pt-4 mt-4">
            <p className="text-[11px] uppercase tracking-widest text-gray-400 mb-3">Pagamento via PIX</p>
            <div className="flex flex-col items-center gap-3">
              <div className="border border-gray-200 p-2 bg-white shadow-sm">
                <img src="/frame.png" alt="QR Code PIX" width={140} height={140} className="block" />
              </div>
              <div className="text-center space-y-0.5">
                <p className="text-[11px] font-semibold text-gray-700">PIX CNPJ</p>
                <p className="text-[11px] text-gray-500 font-mono select-all cursor-copy">01.244.200/0001-52</p>
                <p className="text-[11px] text-gray-400 mt-1">Valor: <span className="font-semibold text-gray-700">{p.parcela}</span></p>
              </div>
              <div className="w-full border border-gray-100 bg-gray-50 rounded-sm px-3 py-2.5 space-y-1.5 text-[11px] text-gray-600">
                <p className="font-semibold text-gray-700 uppercase tracking-wide text-[10px] mb-2">Dados Bancários</p>
                {([["Banco", "Bradesco"], ["Agência", "2522-4 (Nova Suíça)"], ["Conta", "118453-9"]] as const).map(([k, v]) => (
                  <div key={k} className="flex justify-between">
                    <span className="text-gray-400">{k}</span>
                ...
                <div className="flex justify-between border-t border-gray-200 pt-1.5 mt-0.5">
                  ...
                  <span className="font-medium font-mono select-all cursor-copy">01.244.200/0001-52</span>
                </div>
              </div>
              <a href="https://wa.me/5531992118500" target="_blank" rel="noopener noreferrer"
  ...
  return (
    <>
      ...
      {/*
       * Elemento fantasma para geração de PDF.
       * Renderizado via createPortal diretamente no <body>, fora de qualquer
       * container com overflow:hidden ou display:none.
       * - position:fixed + top:-99999px → fora da tela, mas com layout calculado
       * - width:672px fixo → equivale ao max-w-2xl do contrato no desktop
       * - zIndex:-9999 + pointerEvents:none → completamente inerte na UI
       * - aria-hidden:true → invisível para screen readers
       *
       * Isso garante que contractRefPDF.current.offsetWidth === 672 em qualquer
       * dispositivo, produzindo sempre 4 páginas A4 no PDF gerado.
       */}
      {isMounted && createPortal(
        ...
        >
          <div ref={contractRefPDF}>{contractContent}</div>
        </div>,
        document.body
      )}

      <div
        className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-0 md:p-6 pointer-events-none"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div
          ref={modalRef}
          tabIndex={-1}
          className="bg-white w-full max-w-6xl h-full md:h-[95vh] flex flex-col shadow-2xl md:rounded overflow-hidden outline-none pointer-events-auto"
        >
          {/* MOBILE: tab bar */}
          <div className="md:hidden flex shrink-0 border-b border-gray-200 bg-white">
            <button type="button" onClick={() => setMobileTab("contrato")}
              className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-xs font-semibold uppercase tracking-widest transition-colors cursor-pointer ${mobileTab === "contrato" ? "text-gray-900 border-b-2 border-gray-900" : "text-gray-400 hover:text-gray-600"}`}>
              <AlignLeft size={14} /> Contrato
            </button>
            <button type="button" onClick={() => setMobileTab("dados")}
              className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-xs font-semibold uppercase tracking-widest transition-colors cursor-pointer ${mobileTab === "dados" ? "text-gray-900 border-b-2 border-gray-900" : "text-gray-400 hover:text-gray-600"}`}>
              <PenLine size={14} /> Preencher
            </button>
            <button onClick={onClose} className="px-4 text-gray-400 hover:text-gray-700 transition cursor-pointer" aria-label="Fechar">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="md:hidden flex-1 overflow-y-auto flex flex-col">
            {mobileTab === "contrato"
              ? renderDocumentPanel(contractRefVisible)
              : renderFormPanel()}
          </div>
