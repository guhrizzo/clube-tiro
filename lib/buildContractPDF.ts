...
export async function generateContractPDFBase64(contract: ContractData): Promise<string> {
  try {
    // Tenta usar dom-to-image para renderizar com os estilos do site
    const domtoimage = (await import("dom-to-image-more")).default;
    
    // Cria um elemento temporário com o contrato HTML usando os mesmos estilos do site
    const contractDiv = document.createElement("div");
    contractDiv.className = "max-w-170 mx-auto bg-white border border-gray-200 px-6 md:px-10 py-12 text-[13px] text-gray-800 leading-relaxed font-serif shadow-sm";
    ...
    // Renderiza o conteúdo HTML no div
    ...
    // Fallback para o método antigo se dom-to-image falhar
...
export async function generateContractPDFBase64FromElement(element: HTMLElement): Promise<string> {
  ...
  // Clona o elemento para processamento
  const clone = element.cloneNode(true) as HTMLElement;

  // Processa estilos - copia estilos computados do original
...
function getContractHTML(contract: ContractData): string {
  ...
  const dataAssinatura = contract.dataAssinatura?.toDate?.() || new Date(contract.dataAssinaturaString || new Date());
  ...
  return `
    <div style="text-align: center; margin-bottom: 32px; padding-bottom: 24px; border-bottom: 1px solid #e5e7eb;">
      ...
      <div style="margin-top: 16px; display: inline-block; border: 1px solid #d1d5db; padding: 8px 12px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: #4b5563;">
    ...
    <div style="margin-top: 16px; margin-bottom: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      <h2 style="font-size: 13px; font-weight: bold; margin: 0 0 12px;">1. DO PREÇO E DA FORMA DE PAGAMENTO</h2>
      
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA PRIMEIRA:</strong> O valor total do contrato, correspondente à cota ${nomeContrato} (vigente por ${nomePrazo} anos), é de ${total}, ficando isento o pagamento de taxa de contribuição social mensal.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO PRIMEIRO:</strong> Declara o sócio usuário (colaborador), neste ato, ter ciência de que a revalidação do CR (Certificado de Registro) é feita, atualmente, de três em três anos.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO SEGUNDO:</strong> O pagamento deverá ser efetuado no ato da assinatura deste contrato, no valor de ${parcela} (anuidade), correspondente à primeira parcela, salvo eventual desconto ou condição diferenciada. Em caso de parcelamento, o valor remanescente será reajustado ao final de cada ano pelo IGP-M ou índice substituto. Independentemente da data de ingresso, o pagamento será anual e válido até o último dia de dezembro. Este contrato também servirá como recibo, mediante comprovação de depósito via PIX CNPJ nº 01.244.200/0001-52 ou por link de cartão de crédito do clube.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO TERCEIRO:</strong> Caso a contratação seja realizada em mês diverso de janeiro, o contrato permanecerá vigente até o mesmo mês de janeiro, ${nomePrazo} anos após o ingresso, sendo que as parcelas remanescentes também vencerão até o dia 10 daquele mês, em cada um dos ${nomePrazo} anos subsequentes.
      </p>
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO QUINTO:</strong> Nos casos de parcelamento, a cobrança se dará mediante emissão de boleto bancário. O não recebimento do boleto não exime o sócio de realizar o pagamento na data acordada.
      </p>
    ...
    <div style="margin-top: 16px; margin-bottom: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA SEGUNDA:</strong> Permitir a frequência do sócio usuário (colaborador) às áreas comuns do Clube PROTECT dentro do horário de funcionamento, bem como a utilização dos estandes de tiro, ressalvada a hipótese de o local estar sendo utilizado por determinadas categorias profissionais ou para cursos e capacitação.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA TERCEIRA:</strong> Oferecer condições adequadas para a realização de cursos, testes de tiro e capacitação técnica, atividades sociais, culturais, recreativas e desportivas, bem como manter despachantes no clube para pleitos junto à Polícia Federal e ao Exército, ressalvando que os serviços documentais e de despachante serão cobrados separadamente.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA QUARTA:</strong> Manter as instalações limpas e em permanentes condições de uso; disponibilizar, a título oneroso, alvos e equipamentos obrigatórios para testes e cursos; e disponibilizar, a título gratuito e em caráter de empréstimo, óculos de proteção, abafadores de ouvido e armas do acervo do clube para atiradores com CR válido.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA QUINTA:</strong> Ofertar ao sócio usuário (colaborador), armas de fogo, munição, acessórios, equipamentos de proteção e defesa individual, armas de ar comprimido, airsoft e outros itens para compra ou aquisição. Disponibilizar, sempre que necessário, declaração de que o sócio é filiado à entidade e participa regularmente das atividades.
      </p>
    </div>
    ...
    <div style="margin-top: 16px; margin-bottom: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA SEXTA:</strong> Frequentar, utilizar e participar de todas as opções recreativas, desportivas e culturais, desde que esteja em dia com o pagamento da anuidade. O sócio declara ter ciência de que cursos, testes de capacitação e tiro, bem como algumas opções recreativas, serão disponibilizados a título oneroso, a ser calculado evento a evento.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA SÉTIMA:</strong> Ter prioridade na tramitação de pleitos junto ao Exército e à Polícia Federal; na utilização de estandes; para realização de testes de tiro e capacitação técnica; nas vagas para participação em opções recreativas, desportivas e culturais; e para utilização de equipamentos de proteção individual e armas do acervo do clube.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA OITAVA:</strong> Caso a legislação vigente permita, fazer-se acompanhar de terceiros nas dependências do clube PROTECT, mediante preenchimento e assinatura de termo de compromisso, respondendo o sócio usuário (colaborador) por aqueles convidados que agirem com imperícia, imprudência ou negligência.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA NONA:</strong> Fazer requerimento para obtenção de CR de Colecionador, Atirador ou Caçador, observados os requisitos elencados pelo Estatuto do Desarmamento e portarias vigentes da Polícia Federal e do Exército, sendo necessário ter 25 anos. Antes disso, o interessado poderá obter autorização judicial.
      </p>
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA ONZE:</strong> Pagar pontualmente as parcelas para quitação do título ${nomeContrato} de sócio usuário (colaborador), quando parcelado.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA DOZE:</strong> Obedecer às normas disciplinares e aos horários de frequência às dependências do clube, sendo proibida a ingestão de bebidas alcoólicas e/ou drogas ilícitas nas áreas de tiro.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA TREZE:</strong> O ingresso às áreas de tiro se fará mediante identificação facial e/ou apresentação da carteira social. A condição de sócio usuário (colaborador) não confere direito de propriedade sobre qualquer parcela do patrimônio do clube.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA QUATORZE:</strong> O teste de manuseio e capacitação técnica constitui espécie de curso com testes escritos e práticos, ministrados por instrutores credenciados pela Polícia Federal, dos quais o interessado deverá participar e ser aprovado para obtenção ou renovação de CR.
      </p>
    </div>
    ...
    <div style="margin-top: 16px; margin-bottom: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA DEZESSEIS:</strong> O sócio usuário (colaborador) que desejar adquirir uma arma para defesa no comércio deverá obter autorização da Polícia Federal ou do Exército e apresentar Prova de Aptidão e Manuseio de Armas e Teste Psicológico.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA DEZESSETE:</strong> O sócio declara ter ciência de que a legislação que controla os CACs é diferente daquela que rege as armas em mãos de cidadãos comuns. Essas normas constam no R-105, Decreto nº 3.665/2000 e Portaria COLOG nº 051/2015.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA DEZOITO:</strong> O sócio declara ter ciência de que CACs não possuem autorização para PORTE de arma, e sim, alguns deles, para POSSE e TRANSPORTE da arma, munições e acessórios de casa até os locais de competição, clubes de tiro, treinamentos e outros, autorizados pelas GUIAS DE TRÁFEGO.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA VINTE:</strong> É expressamente proibido ao sócio usuário (colaborador) utilizar armas de fogo sem registro, bem como utilizar os postos de tiro sem equipamentos de proteção auricular e visual.
      </p>
    </div>
    ...
    <div style="margin-top: 16px; margin-bottom: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>CLÁUSULA VINTE E UM:</strong> A rescisão ou cancelamento do presente contrato poderá se dar, em qualquer momento, por qualquer uma das partes, mediante termo expresso.
      </p>
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO SEGUNDO:</strong> Em caso de rescisão ou cancelamento deste contrato, a parte que der ensejo à rescisão deverá pagar à outra parte 30% (trinta por cento) do valor da cota ${nomeContrato} de sócio usuário (colaborador), independentemente da entrada e/ou das parcelas pagas, quantia que será liquidada no ato da rescisão ou do cancelamento.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO TERCEIRO:</strong> O presente contrato reveste-se e é aceito pelos contratantes com força de título executivo extrajudicial, constituindo dívida líquida, certa e exigível.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        <strong>PARÁGRAFO QUARTO:</strong> A contratada PROTECT não se responsabiliza por promessas ou acordos que não façam parte deste contrato.
      </p>
    </div>
    ...
    <div style="margin-top: 16px; margin-bottom: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="text-align: justify; margin-bottom: 12px;">
        É expressamente proibido o ingresso e a utilização de armas sem registro no SIGMA ou no SINARM. Qualquer sócio, monitor ou instrutor poderá solicitar aos sócios os documentos relativos às armas trazidas ao clube. É obrigatório transportar as armas desmuniciadas nas dependências do clube, sendo vedado o manejo das armas fora do estande de tiro. A prática de atividades de tiro por menores de 18 anos deverá ser autorizada judicialmente e acompanhada do responsável legal.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        Quando da prática da modalidade de tiro, deverão ser observadas as normas de conduta e segurança, bem como as orientações expedidas pelo Exército Brasileiro, sendo obrigatório o uso de óculos e protetores auriculares.
      </p>

      <p style="text-align: justify; margin-bottom: 12px; font-weight: bold;">
        É EXPRESSAMENTE PROIBIDO O INGRESSO E A UTILIZAÇÃO DE ARMAS E MUNIÇÕES SEM PROCEDÊNCIA LEGAL E JUSTIFICADA.
      </p>

      <p style="text-align: justify; margin-bottom: 12px;">
        Eu, que abaixo assino, declaro ter recebido instruções de segurança e ter tomado conhecimento das normas legais estabelecidas em legislação pertinente, assumindo o compromisso pela minha participação nas atividades e pela minha permanência nas dependências da PROTECT. Declaro, ainda, que não possuo registro de antecedentes criminais e que os dados constantes nesta ficha são verdadeiros. Declaro ter ciência da necessidade de cumprir a Lei nº 10.826/2003, Decreto nº 5.123/2004, R-105 do Exército Brasileiro e demais normas aplicáveis.
      </p>
    </div>
...
export function buildPDFContent(
  ...
  const ML = 15, MR = 15;
  const TW = W - ML - MR;
  const BOTTOM = H - 18; // margem inferior generosa para evitar cortes
  let y = 20;
  ...
  // ── Parse do plano ────────────────────────────────────────────────────────
  // Aceita tanto "5 anos" / "3 anos" quanto a chave numérica "5" / "3"
  const planoStr = contract.plano || "3 anos";
  const isPlano5 = planoStr.includes("5");
  const nomeContrato = isPlano5 ? "quinquenal" : "trienal";
  const nomePrazo = isPlano5 ? "cinco" : "três";
  ...
  /**
   * Converte font-size em pt para mm de altura de linha.
   * Factor: 1pt = 0.352778mm; espaçamento de linha ~1.18 para Times.
   */
  ...
  const para = (str: string, opts: any = {}) => {
    ...
    // Verifica se o bloco inteiro cabe — se não, quebra página
  ...
  const clause = (title: string, body: string, gap = 3) => {
    ...
    lines.forEach((line, i) => {
      if (i === 0) {
        // Título em negrito + corpo na mesma linha
  ...
  text(
    ...
    W / 2,
    y,
    { size: 8, align: "center", color: [80, 80, 80] }
  );
  y += 3;
  ...
  clause(
    "CLÁUSULA PRIMEIRA:",
    `O valor total do contrato, correspondente à cota ${nomeContrato} (vigente por ${nomePrazo} anos), é de ${total}, ficando isento o pagamento de taxa de contribuição social mensal.`
  );
  clause(
    "PARÁGRAFO PRIMEIRO:",
    "Declara o sócio usuário (colaborador), neste ato, ter ciência de que a revalidação do CR (Certificado de Registro) é feita, atualmente, de três em três anos."
  );
  clause(
    "PARÁGRAFO SEGUNDO:",
    `O pagamento deverá ser efetuado no ato da assinatura deste contrato, no valor de ${parcela} (anuidade), correspondente à primeira parcela, salvo eventual desconto ou condição diferenciada. Em caso de parcelamento, o valor remanescente será reajustado ao final de cada ano pelo IGP-M ou índice substituto. Independentemente da data de ingresso, o pagamento será anual e válido até o último dia de dezembro. Este contrato também servirá como recibo, mediante comprovação de depósito via PIX CNPJ nº 01.244.200/0001-52 ou por link de cartão de crédito do clube.`
  );
  clause(
    "PARÁGRAFO TERCEIRO:",
    `Caso a contratação seja realizada em mês diverso de janeiro, o contrato permanecerá vigente até o mesmo mês de janeiro, ${nomePrazo} anos após o ingresso, sendo que as parcelas remanescentes também vencerão até o dia 10 daquele mês, em cada um dos ${nomePrazo} anos subsequentes.`
  );
  clause(
    "PARÁGRAFO QUARTO:",
    "Será considerado inadimplente o sócio usuário (colaborador) que atrasar qualquer pagamento por período superior a 30 (trinta) dias do vencimento, ficando expressamente suspensos os direitos previstos nas cláusulas sexta e sétima, independentemente de comunicação prévia, até a quitação do débito."
  );
  clause(
    "PARÁGRAFO QUINTO:",
    "Nos casos de parcelamento, a cobrança se dará mediante emissão de boleto bancário. O não recebimento do boleto não exime o sócio de realizar o pagamento na data acordada."
  );
  clause(
    "PARÁGRAFO SEXTO:",
    `Em caso de atraso superior a 60 (sessenta) dias, o sócio será notificado e terá 10 (dez) dias para liquidação. Persistindo a pendência, a PROTECT poderá encaminhar o nome ao SPC. O cancelamento ou rescisão implicará o vencimento antecipado das parcelas remanescentes, acrescidas de multa de 30% sobre o valor total da cota ${nomeContrato}, além de correção monetária. Cobrança administrativa: +10% de honorários; cobrança judicial: +20% de honorários.`
  );
  ...
  clause(
    "CLÁUSULA SEGUNDA:",
    "Permitir a frequência do sócio usuário (colaborador) às áreas comuns do Clube PROTECT dentro do horário de funcionamento, bem como a utilização dos estandes de tiro, ressalvada a hipótese de o local estar sendo utilizado por determinadas categorias profissionais ou para cursos e capacitação."
  );
  clause(
    "CLÁUSULA TERCEIRA:",
    "Oferecer condições adequadas para a realização de cursos, testes de tiro e capacitação técnica, atividades sociais, culturais, recreativas e desportivas, bem como manter despachantes no clube para pleitos junto à Polícia Federal e ao Exército, ressalvando que os serviços documentais e de despachante serão cobrados separadamente."
  );
  clause(
    "CLÁUSULA QUARTA:",
    "Manter as instalações limpas e em permanentes condições de uso; disponibilizar, a título oneroso, alvos e equipamentos obrigatórios para testes e cursos; e disponibilizar, a título gratuito e em caráter de empréstimo, óculos de proteção, abafadores de ouvido e armas do acervo do clube para atiradores com CR válido."
  );
  clause(
    "CLÁUSULA QUINTA:",
    "Ofertar ao sócio usuário (colaborador), armas de fogo, munição, acessórios, equipamentos de proteção e defesa individual, armas de ar comprimido, airsoft e outros itens para compra ou aquisição. Disponibilizar, sempre que necessário, declaração de que o sócio é filiado à entidade e participa regularmente das atividades."
  );
  ...
  clause(
    "CLÁUSULA SEXTA:",
    "Frequentar, utilizar e participar de todas as opções recreativas, desportivas e culturais, desde que esteja em dia com o pagamento da anuidade. O sócio declara ter ciência de que cursos, testes de capacitação e tiro, bem como algumas opções recreativas, serão disponibilizados a título oneroso, a ser calculado evento a evento."
  );
  clause(
    "CLÁUSULA SÉTIMA:",
    "Ter prioridade na tramitação de pleitos junto ao Exército e à Polícia Federal; na utilização de estandes; para realização de testes de tiro e capacitação técnica; nas vagas para participação em opções recreativas, desportivas e culturais; e para utilização de equipamentos de proteção individual e armas do acervo do clube."
  );
  clause(
    "CLÁUSULA OITAVA:",
    "Caso a legislação vigente permita, fazer-se acompanhar de terceiros nas dependências do clube PROTECT, mediante preenchimento e assinatura de termo de compromisso, respondendo o sócio usuário (colaborador) por aqueles convidados que agirem com imperícia, imprudência ou negligência."
  );
  clause(
    "CLÁUSULA NONA:",
    "Fazer requerimento para obtenção de CR de Colecionador, Atirador ou Caçador, observados os requisitos elencados pelo Estatuto do Desarmamento e portarias vigentes da Polícia Federal e do Exército, sendo necessário ter 25 anos. Antes disso, o interessado poderá obter autorização judicial."
  );
  clause(
    "CLÁUSULA DÉCIMA:",
    "Zelar pelo patrimônio do clube, responsabilizando-se por si e por seus convidados, inclusive por danos ou despesas que venham a causar. É expressamente proibido que menores de 18 anos manuseiem, utilizem ou portem qualquer tipo de arma de fogo, à exceção daqueles com autorização judicial."
  );
  clause(
    "CLÁUSULA ONZE:",
    `Pagar pontualmente as parcelas para quitação do título ${nomeContrato} de sócio usuário (colaborador), quando parcelado.`
  );
  clause(
    "CLÁUSULA DOZE:",
    "Obedecer às normas disciplinares e aos horários de frequência às dependências do clube, sendo proibida a ingestão de bebidas alcoólicas e/ou drogas ilícitas nas áreas de tiro."
  );
  clause(
    "CLÁUSULA TREZE:",
    "O ingresso às áreas de tiro se fará mediante identificação facial e/ou apresentação da carteira social. A condição de sócio usuário (colaborador) não confere direito de propriedade sobre qualquer parcela do patrimônio do clube."
  );
  clause(
    "CLÁUSULA QUATORZE:",
    "O teste de manuseio e capacitação técnica constitui espécie de curso com testes escritos e práticos, ministrados por instrutores credenciados pela Polícia Federal, dos quais o interessado deverá participar e ser aprovado para obtenção ou renovação de CR."
  );
  ...
  clause(
    "CLÁUSULA DEZESSEIS:",
    "O sócio usuário (colaborador) que desejar adquirir uma arma para defesa no comércio deverá obter autorização da Polícia Federal ou do Exército e apresentar Prova de Aptidão e Manuseio de Armas e Teste Psicológico."
  );
  clause(
    "CLÁUSULA DEZESSETE:",
    "O sócio declara ter ciência de que a legislação que controla os CACs é diferente daquela que rege as armas em mãos de cidadãos comuns. Essas normas constam no R-105, Decreto nº 3.665/2000 e Portaria COLOG nº 051/2015."
  );
  clause(
    "CLÁUSULA DEZOITO:",
    "O sócio declara ter ciência de que CACs não possuem autorização para PORTE de arma, e sim, alguns deles, para POSSE e TRANSPORTE da arma, munições e acessórios de casa até os locais de competição, clubes de tiro, treinamentos e outros, autorizados pelas GUIAS DE TRÁFEGO."
  );
  clause(
    "CLÁUSULA VINTE:",
    "É expressamente proibido ao sócio usuário (colaborador) utilizar armas de fogo sem registro, bem como utilizar os postos de tiro sem equipamentos de proteção auricular e visual."
  );
  ...
  clause(
    "CLÁUSULA VINTE E UM:",
    "A rescisão ou cancelamento do presente contrato poderá se dar, em qualquer momento, por qualquer uma das partes, mediante termo expresso."
  );
  clause(
    "PARÁGRAFO PRIMEIRO:",
    "Não será considerada motivação válida para rescisão contratual a não aprovação do sócio usuário (colaborador) nos testes de capacitação técnica e manuseio de armas de fogo e nos testes psicotécnicos pertinentes."
  );
  clause(
    "PARÁGRAFO SEGUNDO:",
    `Em caso de rescisão ou cancelamento deste contrato, a parte que der ensejo à rescisão deverá pagar à outra parte 30% (trinta por cento) do valor da cota ${nomeContrato} de sócio usuário (colaborador), independentemente da entrada e/ou das parcelas pagas, quantia que será liquidada no ato da rescisão ou do cancelamento.`
  );
  clause(
    "PARÁGRAFO TERCEIRO:",
    "O presente contrato reveste-se e é aceito pelos contratantes com força de título executivo extrajudicial, constituindo dívida líquida, certa e exigível."
  );
  clause(
    "PARÁGRAFO QUARTO:",
    "A contratada PROTECT não se responsabiliza por promessas ou acordos que não façam parte deste contrato."
  );
  ...
  // Assinatura PROTECT (s2)
  try {
    pdf.addImage(sigs.s2, "PNG", ML, y, sigW, sigH, undefined, "FAST");
  } catch (_) {}
  ...
  // Testemunhas
  try {
    pdf.addImage(sigs.s1, "PNG", ML, y, sigW, sigH, undefined, "FAST");
  } catch (_) {}
  try {
    pdf.addImage(sigs.s3, "PNG", ML + col + 6, y, sigW, sigH, undefined, "FAST");
  } catch (_) {}
