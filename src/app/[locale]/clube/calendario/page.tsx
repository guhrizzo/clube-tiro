...
export default function AgendaClient() {
  ...
  // Acessando o dicionário usando o idioma do contexto
  const t = dictionaries[lang].agenda; 
  ...
  const categoriasLegenda = [
    { label: t.legend.exams, bg: "bg-amber-400" },
    { label: t.legend.courses, bg: "bg-red-500" },
    { label: t.legend.tests, bg: "bg-blue-500" },
    { label: t.legend.events, bg: "bg-green-500" },
  ];
    ...
    <div className="min-h-screen bg-slate-50/50 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
        <div className="text-center mb-10">
          <span className="text-yellow-600 font-bold uppercase tracking-widest text-xs italic">
            {t.badge}
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1 space-y-6 order-2 lg:order-1">
            <div className="bg-white p-6 rounded-4xl border border-slate-100 shadow-sm">
              <h3 className="font-bold text-slate-900 text-xs mb-4 flex items-center gap-2 uppercase tracking-widest">
                <Tag size={14} className="text-yellow-600" /> {t.legend_title}
              </h3>
            ...
            <div className="bg-slate-900 p-6 rounded-4xl text-white">
               ...
               <p className="text-xs text-slate-400">{t.click_notice}</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 bg-white p-4 md:p-6 rounded-[2.5rem] border border-slate-100 shadow-2xl relative order-1 lg:order-2">
            ...
            <FullCalendar
              ...
              buttonText={{ 
                month: lang === 'pt' ? 'Mês' : (lang === 'en' ? 'Month' : 'Mes'), 
                listMonth: lang === 'pt' ? 'Lista' : (lang === 'en' ? 'List' : 'Lista') 
              }}
            />
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-8 space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] uppercase font-black text-slate-400">{t.modal.date_label}</p>
                  <p className="font-bold text-slate-800">
                    {selectedEvent.start?.toLocaleDateString(lang === 'pt' ? 'pt-BR' : (lang === 'en' ? 'en-US' : 'es-ES'), { dateStyle: 'long' })}
                  </p>
                </div>
              </div>
              ...
              <button onClick={() => setIsModalOpen(false)} className="w-full bg-slate-950 text-white py-4 rounded-2xl font-black uppercase text-[10px] cursor-pointer">
